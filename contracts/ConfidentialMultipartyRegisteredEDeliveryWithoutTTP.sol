...
contract ConfidentialMultipartyRegisteredEDeliveryWithoutTTPFactory {
    ...
    function createDelivery(address _receiver, bytes _c1, bytes _c2, bytes _ya, bytes _g, bytes _p, uint _term) public payable {
        address newDelivery = (new ConfidentialMultipartyRegisteredEDeliveryWithoutTTP)
            .value(msg.value)(msg.sender, _receiver, _c1, _c2, _ya, _g, _p, _term);
        deliveries.push(newDelivery);
        senderDeliveries[msg.sender].push(newDelivery);
        receiverDeliveries[_receiver].push(newDelivery);
...
contract ConfidentialMultipartyRegisteredEDeliveryWithoutTTP {
    ...
    mapping(address => estados) estado;
    ...
    constructor (address _sender, address _receiver, bytes _c1, bytes _c2, bytes _ya, bytes _g, bytes _p, uint _term) public payable {
        ...
        sender = _sender;
        receiver = _receiver;
        // We set the state of receiver to 'created'
        estado[address(this)].state = State.created;
        c1 = _c1;
        c2 = _c2;
        ya = _ya;
        g = _g;
    ...
    function challenge(bytes _z1,bytes _z2, bytes _yb, bytes _c)public{
        ...
        require(receiver==msg.sender,"who try to read the contract is not the original receptor");
        require(estado[address(this)].state==State.created,"The process is not in the adequate state");
        z1=_z1;
        z2=_z2;
        yb=_yb;
        c=_c;
        estado[address(this)].state=State.challenged;
    ...
    function response(bytes _w) public{
        ...
        w=_w;
        bytes memory check_1 = bignumber_modexp(g,w,p);
        bytes memory check_2 = bignumber_modmul(c1, bignumber_modexp(yb,c,p), p);
        require(bignumber_equals(check_1,check_2),"(g^w_mod_p) y ((g^r mod p)(ybc mod p)mod p) are not equals");
        estado[address(this)].state=State.responsed;
    }
    ...
    function accept() public {
        require(now < start+term, "The timeout has been reached");
        require(receiver==msg.sender,"who try to read the contract is not the original receptor");
        require(estado[address(this)].state==State.responsed, "The process is not in the adequate state");
        estado[address(this)].state=State.accepted;
    }
    ...
    function finish() public {
        require(now < start+term,"The timeout has been reached");
        require(sender==msg.sender,"Who try to finish the contract signature is not the proposer");
        require(estado[address(this)].state==State.accepted,"The process is not in the adequate state");
        sender.transfer(address(this).balance);
        estado[address(this)].state=State.finished;
    }


    function cancelA() public {
        ...
        require((estado[address(this)].state==State.created)||(estado[address(this)].state==State.responsed),"The process is not in the adequate state");
    ...
    function cancelB() public {
        ...
        require((estado[address(this)].state==State.accepted)||(estado[address(this)].state==State.challenged),"The process is not in the adequate state");
        estado[address(this)].state=State.cancelled;
    }

    // getState() returns the state of a receiver in an string format
    function getState(address _contract) public view returns (string) {
        if (estado[_contract].state==State.notexists) {
            return "not exists";
            ...
            return "cancelled";
        } else if (estado[_contract].state==State.accepted) {
            return "accepted";
        } else if (estado[_contract].state==State.finished) {
            return "finished";
        } else if (estado[_contract].state==State.rejected) {
            return "rejected";
        }
    }
